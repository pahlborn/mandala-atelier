...
const Speicher = {
  ...
  open: function () {
    ...
    return new Promise(function (resolve) {
      let request;
      try {
        request = indexedDB.open('atelier3-regal3-2', 1);
      } catch (err) {
        resolve(false);
        return;
      }
...
function bindUi() {
  ...
  ui.traySound.addEventListener('click', function () {
    ...
    try { localStorage.setItem('atelier3-regal3-2-ton', Klang.muted ? 'aus' : 'an'); } catch (err) {}
    syncSoundLabel();
  });
...
async function start() {
  ...
  try { Klang.muted = localStorage.getItem('atelier3-regal3-2-ton') === 'aus'; } catch (err) {}
  syncSoundLabel();
  ...
  let seen = false;
  try { seen = localStorage.getItem('atelier3-regal3-2-gesehen') === 'ja'; } catch (err) {}
  if (!seen && !sheet.touched) {
    ...
    const dismiss = function () {
      ...
      try { localStorage.setItem('atelier3-regal3-2-gesehen', 'ja'); } catch (err) {}
