...
const Store = {
  prefix: 'mandala-atelier-regal2-32.',
  get(key, fallback) {
    try {
...
MOTIFS.push(
  ...
  {
    id: 'gitterschale', world: 'geo', axes: 12,
    name: 'Gitterschale', note: 'Zwei Rautengitter, über die Bandgrenze geschoben',
    mythos: 'Ein Gitter hält nichts fest. Es zeigt nur, dass es eine Ordnung gibt, und lässt alles durch, was hindurch will.',
    build: function (p) {
      drawRing(p, 272, 1.2);
      drawRing(p, 168, 1.2);
      drawRing(p, 104, 1.2);
      drawClosedLoop(p, diamondPoints(104, 272, p.step * 0.46), 1.7);
      drawClosedLoop(p, konturPoints(diamondPoints(104, 272, p.step * 0.46), 0.52), 1);
      drawClosedLoop(p, halb(p, diamondPoints(168, 406, p.step * 0.46)), 1.7);
      drawClosedLoop(p, halb(p, konturPoints(diamondPoints(168, 406, p.step * 0.46), 0.52)), 1);
      drawSprossen(p, 272, 406, p.step * 0.30, 3, 0.9);
      drawSprossen(p, 46, 104, p.step * 0.30, 3, 0.9, p.step / 2);
      drawPerlen(p, 75, p.step * 0.20, 2, 7, 1.1);
      drawPerlen(p, 340, p.step * 0.24, 3, 7, 1.1, p.step / 2);
      drawClosedLoop(p, petalPoints(14, 44, p.step * 0.88, 10, 0.8), 1.5);
    }
  },
  {
    id: 'granulat', world: 'feinwerk', axes: 12,
    name: 'Granulat', note: 'Rautennetz in zwei Lagen, vier Perlreihen dazwischen',
    mythos: 'Tausend Kügelchen, jedes einzeln gesetzt, keines wichtiger als das andere. So entsteht Glanz: nicht durch ein großes Licht, sondern dadurch, dass sehr viele sehr kleine Dinge in dieselbe Richtung schauen.',
    build: function (p) {
      drawRing(p, 340, 1.2);
      drawRing(p, 268, 1.2);
      drawRing(p, 196, 1.2);
      drawRing(p, 132, 1.2);
      drawClosedLoop(p, diamondPoints(132, 268, p.step * 0.42), 1.7);
      drawClosedLoop(p, konturPoints(diamondPoints(132, 268, p.step * 0.42), 0.58), 1);
      drawClosedLoop(p, halb(p, diamondPoints(268, 406, p.step * 0.42)), 1.7);
      drawClosedLoop(p, halb(p, konturPoints(diamondPoints(268, 406, p.step * 0.42), 0.58)), 1);
      drawPerlen(p, 96, p.step * 0.229, 3, 10, 1.3);
      drawPerlen(p, 164, p.step * 0.306, 4, 8, 1.2, p.step / 2);
      drawPerlen(p, 304, p.step * 0.267, 4, 8, 1.2);
      drawPerlen(p, 374, p.step * 0.325, 5, 7, 1.1, p.step / 2);
      drawSprossen(p, 196, 268, p.step * 0.248, 3, 0.9);
      drawClosedLoop(p, petalPoints(14, 44, p.step * 0.84, 10, 0.8), 1.5);
    }
  },
  {
  ...
  {
    id: 'rankengeflecht', world: 'natur', axes: 8,
    name: 'Rankengeflecht', note: 'Zwei gegenläufige Rankenpaare, drei Blattlagen',
    mythos: 'Eine Ranke sucht nicht das Licht, sie sucht Halt. Dass sie dabei schön wird, ist ein Nebenprodukt — und wahrscheinlich der Grund, warum wir sie nachzeichnen.',
    build: function (p) {
      drawRing(p, 328, 1.1);
      drawRing(p, 236, 1.1);
      drawRing(p, 140, 1.1);
      drawPolyline(p, vinePoints(62, 404, 0.42, 70), 1.9);
      drawPolyline(p, vinePoints(62, 404, -0.42, 70), 1.9);
      drawPolyline(p, halb(p, vinePoints(140, 398, 0.26, 50)), 1);
      drawPolyline(p, halb(p, vinePoints(140, 398, -0.26, 50)), 1);
      drawClosedLoop(p, petalPoints(142, 232, p.step * 0.255, 16, 0.6), 1.4);
      drawClosedLoop(p, konturPoints(petalPoints(142, 232, p.step * 0.255, 16, 0.6), 0.55), 0.9);
      drawClosedLoop(p, halb(p, petalPoints(240, 404, p.step * 0.229, 16, 0.6)), 1.4);
      drawClosedLoop(p, halb(p, konturPoints(petalPoints(240, 404, p.step * 0.229, 16, 0.6), 0.55)), 0.9);
      drawClosedLoop(p, halb(p, petalPoints(62, 138, p.step * 0.28, 14, 0.7)), 1.3);
      drawClosedLoop(p, halb(p, konturPoints(petalPoints(62, 138, p.step * 0.28, 14, 0.7), 0.52)), 0.9);
      drawPerlen(p, 300, p.step * 0.204, 3, 8, 1.2);
      drawPerlen(p, 104, p.step * 0.178, 2, 7, 1.2);
      drawPerlen(p, 370, p.step * 0.166, 3, 7, 1.1, p.step / 2);
      drawClosedLoop(p, petalPoints(14, 44, p.step * 0.64, 10, 0.8), 1.5);
    }
  },
  {
  ...
  {
    id: 'kettenreif', world: 'feinwerk', axes: 10,
    name: 'Kettenreif', note: 'Vier Reihen Glieder, jede in die darunterliegende gehängt',
    ...
    build: function (p) {
      drawClosedLoop(p, ovalPoints(96, 54, p.step * 0.40), 1.7);
      drawClosedLoop(p, konturPoints(ovalPoints(96, 54, p.step * 0.40), 0.55), 1);
      drawClosedLoop(p, halb(p, ovalPoints(172, 60, p.step * 0.40)), 1.7);
      drawClosedLoop(p, halb(p, konturPoints(ovalPoints(172, 60, p.step * 0.40), 0.55)), 1);
      drawClosedLoop(p, ovalPoints(252, 62, p.step * 0.40), 1.7);
      drawClosedLoop(p, konturPoints(ovalPoints(252, 62, p.step * 0.40), 0.55), 1);
      drawClosedLoop(p, halb(p, ovalPoints(334, 62, p.step * 0.40)), 1.7);
      drawClosedLoop(p, halb(p, konturPoints(ovalPoints(334, 62, p.step * 0.40), 0.55)), 1);
      drawClosedLoop(p, ovalPoints(398, 26, p.step * 0.26), 1.4);
      drawRing(p, 300, 0.9);
      drawRing(p, 134, 0.9);
      drawPerlen(p, 216, p.step * 0.22, 2, 8, 1.2);
      drawPerlen(p, 368, p.step * 0.18, 2, 7, 1.1, p.step / 2);
      drawClosedLoop(p, petalPoints(14, 46, p.step * 0.7, 10, 0.8), 1.5);
    }
  },
  {
    id: 'schuppenwerk', world: 'feinwerk', axes: 16,
    name: 'Schuppenwerk', note: 'Fünf Reihen Schuppen, jede über der Naht der darunter',
    mythos: 'Jede Schuppe deckt die Naht der vorigen. So wird aus lauter kleinen Blechen eine Haut, die nichts durchlässt und sich trotzdem bewegt.',
    build: function (p) {
      drawClosedLoop(p, schuppePoints(48, 132, p.step * 0.62, 18), 1.5);
      drawClosedLoop(p, halb(p, schuppePoints(120, 204, p.step * 0.62, 18)), 1.5);
      drawClosedLoop(p, schuppePoints(192, 276, p.step * 0.62, 18), 1.5);
      drawClosedLoop(p, halb(p, schuppePoints(264, 348, p.step * 0.62, 18)), 1.5);
      drawClosedLoop(p, schuppePoints(336, 408, p.step * 0.62, 18), 1.5);
      drawClosedLoop(p, konturPoints(schuppePoints(48, 132, p.step * 0.62, 18), 0.52), 0.9);
      drawClosedLoop(p, halb(p, konturPoints(schuppePoints(120, 204, p.step * 0.62, 18), 0.52)), 0.9);
      drawClosedLoop(p, konturPoints(schuppePoints(192, 276, p.step * 0.62, 18), 0.52), 0.9);
      drawClosedLoop(p, halb(p, konturPoints(schuppePoints(264, 348, p.step * 0.62, 18), 0.52)), 0.9);
      drawClosedLoop(p, konturPoints(schuppePoints(336, 408, p.step * 0.62, 18), 0.52), 0.9);
      drawClosedLoop(p, petalPoints(14, 44, p.step * 1.0, 10, 0.8), 1.4);
    }
  },
  {
...
const Gallery = {
  ...
  open: function () {
    ...
    return new Promise(function (resolve) {
      ...
      let request;
      try {
        request = indexedDB.open('mandala-atelier-regal2-32', 1);
      } catch (err) {
        return resolve();
      }
