...
/* Die Griffkurve — und warum die Voreinstellung heute eine andere ist.
   ...
   was `zeichnend` beim Trödeln liefert — die Trennschärfe ist dort also bei
   keiner Geschwindigkeit zu erreichen. Nur: Sie war nicht das, wonach
   gesucht wurde. Wer reibt, will offenbar, dass etwas kommt.

   **Die Voreinstellung ist `flaechig`** — und dahin ist sie in zwei Schritten
   gekommen, die beide von einem Handtest am iPad entschieden wurden:

     v1-12  auf `zeichnend`, nach dem ersten Test: „Blatt damals schenkt
            einem die beste Malfreude."
     v1-15  zurück auf `flaechig`, nach dem zweiten, bei dem alle vier Griffe
            in derselben Fassung nebeneinander lagen: „flächig erzeugt die
            größte Malfreude."

   Das ist kein Hin und Her, sondern der Unterschied zwischen einer Erinnerung
   an eine alte Fassung und einem Vergleich unter gleichen Bedingungen. Was
   sich hier rechnen lässt, ist die Trennschärfe; was zählt, ist die Freude
   beim Malen, und die kann nur die Hand beantworten.

   Alle vier Griffe bleiben über `?griff=` erreichbar — ausdrücklich für
   spätere Versuche mit anderen Geräten und anderen Menschen. Ein Finger ist
   nicht jeder Finger. Die alten Namen `damals` und `jetzt` bleiben als
   Verweise gültig, damit Adressen aus der Vergleichsphase nicht ins Leere
   laufen.
...
const GRIFF_VOREINSTELLUNG = 'flaechig';
...
/* Das Blatt bewegen. Der Anschlag stand bei 2,2, und die Begründung war die
   Schärfe: Das Blatt hat 1280 Punkte Kantenlänge, jenseits davon sieht man
   das Papierkorn statt feinerer Blüten.

   Der Anschlag ist jetzt 4, und der Grund wiegt schwerer als die Schärfe:
   GENAUIGKEIT. Der Pinsel ist so fein wie das, was ein Finger auf dem Glas
   bedeckt – contactRadius() rechnet R_FINGER_CSS über view.perCss in
   Blatt-Einheiten um, und perCss fällt linear mit dem Zoom. Nachgerechnet
   an einem iPad-Fenster, in dem das Blatt 447 Bildschirmpunkte breit liegt:

     Zoom 1,0   Finger deckt 166 Blatt-Einheiten
     Zoom 2,2   Finger deckt  76 Blatt-Einheiten
     Zoom 4,0   Finger deckt  42 Blatt-Einheiten

   Ein Band der Stimmung „Feinwerk“ ist rund 60 Einheiten breit. Bei 2,2
   deckte der Finger also immer noch mehr als ein ganzes Band – ein sauberer
   Strich innerhalb eines Feldes war nicht möglich, und das Ergebnis sah
   krumm aus. Bei 4,0 passt der Finger hinein.

   Der Preis ist die Schärfe, und er ist echt: Der Zoom ist ein CSS-`scale`
   auf ein Canvas fester Größe, mehr Zoom heißt also mehr Vergrößerung und
   nicht mehr Auflösung. Wer die auch will, muss den Bildspeicher des Canvas
   vergrößern – das ist ein eigener Schritt, denn das Papierkorn wird je
   Bildpunkt gewürfelt und sähe danach anders aus. */
const VIEW_MAX      = 4;
...
/* Die Farbwelten des Mandala Ateliers, Ton für Ton dieselben – ihre Stimmung
   ist das eigentliche Kapital, und sie darf zwischen den beiden Apps nicht
   auseinanderlaufen.

   Zehn je Welt, überall gleich viele, und welche zehn ist gerechnet: Hier
   wird Farbe **durchgerieben**, nicht deckend aufgetragen, und bei Nacht
   zusätzlich in Kreide umgerechnet. Beides schiebt die Töne zusammen. Die
   Auswahl misst deshalb bei 70 % Deckung und in beiden Modi – die Begründung
   steht im Kopf der Farbwelten in ../app.js, der Befund in asChalk() weiter
   oben.
...
/* Dieselbe Welt als Kreide auf getöntem Papier. Nicht von Hand ein zweites
   Mal ausgesucht, sondern abgeleitet: Farbton bleibt, Helligkeit wird
   angehoben, Buntheit leicht zurückgenommen. So bleibt die Stimmung einer
   Welt bei Tag und bei Nacht dieselbe.

   Die erste Fassung setzte **alle** Stifte auf dieselbe Helligkeit (L = 0.74)
   und deckelte die Buntheit bei 0.46. Das war der Fehler, und es war ein
   grober: Wovon sich zwei gedeckte Pigmente hauptsächlich unterscheiden, ist
   die Helligkeit. Wer die einebnet, macht aus zwei Stiften einen. Nachgemessen
   waren Rauch und Asche als Kreide **ΔE00 0.0** — Zeichen für Zeichen
   dasselbe Grau; Ocker und Gold aus dem Goldgrund ebenso. Eine Welt, die bei
   Tag noch vierzehn Töne hatte, war bei Nacht ein Pastellstreifen.

   Jetzt wird die Helligkeit **abgebildet statt gesetzt**: L' = 0.42 + 0.50·L.
   Der dunkelste Stift landet bei 0.42, der hellste bei 0.92 – beides liegt
   deutlich über dem Papier (L ≈ 0.14), leuchtet also als Kreide, und der
   Abstand zwischen zwei Stiften bleibt erhalten. Aus 0.0 werden 2.8. */
function asChalk(rgb) {
  ...
  const L = 0.42 + 0.50 * l;
  const S = Math.min(0.50, sat * 0.90);
...
/* ===== FEINWERK – ANFANG ====================================================
   ...
   Eine sechste Stimmung, angehängt statt hineingeschrieben. Alles zwischen
   diesem Zeichen und „FEINWERK – ENDE“ lässt sich in einem Stück entfernen.

   Warum das sicher ist, und zwar nachgesehen statt vermutet: buildPlan()
   würfelt aus den Vorräten der GEWÄHLTEN Stimmung (art.axes, art.bands,
   art.wide, art.narrow). Die Länge der Liste KINDS geht nirgends in eine
   Rechnung ein – anders als WORLDS.length bei der Farbwelt und
   BAUTEN.length bei den Anlagen. Solange kein bestehender Vorrat angefasst
   wird, würfelt „Ruhe“ morgen wie gestern, und jedes gesicherte Blatt
   rechnet sich unverändert nach. Nachgemessen an 200 Bauplänen (fünf
   Stimmungen, vierzig Seeds): identisch vor und nach dieser Zeile.

   Was die Stimmung anders macht als die vier anderen Kränze: mehr Bänder
   (sechs bis acht statt drei bis sechs), schmalere Striche und ein Vorrat,
   der Perlen und Strahlen hochgewichtet. Das ist die Übersetzung dessen,
   was im Atelier die Motivfamilie Feinwerk tut – nicht als Kopie eines
   Motivs, denn Blatt hat keinen Katalog: dort wählt man, hier bekommt man.

   Zopf, Kette und Zelle fehlen noch. Sie wären neue Bandarten und gehören
   in einen eigenen Schritt mit eigenem Messwert – sicher sind auch sie,
   solange nur diese Stimmung sie zieht.
   ------------------------------------------------------------------------ */

KINDS.push({
  id: 'feinwerk', name: 'Feinwerk',
  axes: [14, 16, 18, 18, 20, 20], bands: [6, 8], lw: [0.0040, 0.0056],
  wide:   ['lattice', 'rays', 'petals', 'lattice', 'scallops'],
  narrow: ['beads', 'rays', 'beads', 'plain']
});
...
function nextThought() {
  let seen = [];
  try { seen = JSON.parse(localStorage.getItem('atelier3-regal3-22-gedanken') || '[]'); } catch (err) {}
  ...
  try { localStorage.setItem('atelier3-regal3-22-gedanken', JSON.stringify(seen)); } catch (err) {}
...
const Speicher = {
  ...
  open: function () {
    ...
    return new Promise(function (resolve) {
      let request;
      try {
        request = indexedDB.open('atelier3-regal3-22', 1);
      } catch (err) {
        resolve(false);
        return;
      }
...
/* Tag oder Nacht ist keine Einstellung – es sind zwei Papiere. Auf dem
   ...
   Papiere selbst. */
const PAPER_KEY = 'atelier3-regal3-22-papier';
...
function bindUi() {
  ...
  ui.traySound.addEventListener('click', function () {
    ...
    try { localStorage.setItem('atelier3-regal3-22-ton', Klang.muted ? 'aus' : 'an'); } catch (err) {}
    syncSoundLabel();
  });
...
async function start() {
  ...
  try { Klang.muted = localStorage.getItem('atelier3-regal3-22-ton') === 'aus'; } catch (err) {}
  syncSoundLabel();
  ...
  let seen = false;
  try { seen = localStorage.getItem('atelier3-regal3-22-gesehen') === 'ja'; } catch (err) {}
  if (!seen && !sheet.touched) {
    showHint(T('Streiche über das Blatt.'), 9000);
    try { localStorage.setItem('atelier3-regal3-22-gesehen', 'ja'); } catch (err) {}
  }
