...
const Store = {
  prefix: 'mandala-atelier-regal2-21.',
  get(key, fallback) {
    try {
...
const state = {
  ...
  legendFarben: {},   // { motivId: { zahl: hex } } – selbst vergebene Farben
  axes:       12,
  mirror:     false,
  guides:     true,
...
/* Zierrand und Bänder der Zähl- und Rechenmandalas. Der Rand bleibt zahlenfrei
   und darf frei bemalt werden – deshalb hören die Bänder bei R_OUT − KIDS_RAND
   auf. Zwei Ringe für die kurzen Blätter, drei für die gestaffelten. */
const KIDS_RAND    = 26;
const KIDS_R       = R_OUT - KIDS_RAND;
const KIDS_INNEN   = [112, 250, 2];       // Zierring innen, doppelt besetzt
const KIDS_AUSSEN  = [250, KIDS_R, 2];    // Aufgabenring außen, doppelt besetzt
...
const MOTIFS = [
  ...
  /* Die Zähl- und Rechenmandalas. Bänder, Verteilung, Mengenbild und
     Schwierigkeit stehen hier als Daten; gezeichnet werden sie alle von
     kidsBuild(). RAND_KIDS ist der Zierrand – die Bänder hören davor auf,
     damit er frei bleibt. */
  {
    id: 'zaehlen6', world: 'kids', axes: 8,
    ...
    task: 'Zähl die Punkte in einem Feld. Unten in der Leiste steht bei jeder '
      + 'Farbe eine Zahl – nimm die Farbe mit deiner Anzahl und tippe ins '
      + 'Feld. Nabe und Rand darfst du frei bemalen.',
    kind: 'count',
    bands: [KIDS_AUSSEN],
    zier: [KIDS_INNEN],
    values: [1, 2, 3, 4, 5, 6],
  ...
  },
  {
    ...
    name: 'Zähl bis 10', note: 'Punkte zählen bis zehn, in Fünferreihen',
    task: 'Zähl die Punkte in einem Feld. Unten in der Leiste steht bei jeder '
      + 'Farbe eine Zahl – nimm die Farbe mit deiner Anzahl und tippe ins '
      + 'Feld. Nabe und Rand darfst du frei bemalen.',
    kind: 'count',
...
/* ---------------------------------------------------------------------------
   Zähl- und Rechenmandalas — die Form

   Zwei Anläufe sind gescheitert, und beide aus demselben Grund.

   Der erste zeichnete zwei Ringe und ließ den Rahmen den Rest tun: zwölf bis
   sechzehn gleiche Tortenstücke, eine Dartscheibe. Der zweite machte die
   Bandgrenzen zu Wellen, sodass aus jedem Tortenstück ein Blatt wurde — und
   setzte in jedes Blatt ein großes Mengenbild. Beide Male stand am Ende ein
   leeres Feld mit einem Zeichen in der Mitte. Ein Arbeitsblatt mit rundem
   Rand ist kein Mandala.

   Der Fehler war, das **Zeichen zur Hauptsache** zu machen. Bei einem guten
   Malen-nach-Zahlen ist die Zahl das Unauffälligste am ganzen Bild; was man
   ansieht, ist das Ornament, und beim Bemalen verschwindet die Zahl darunter.

   Also andersherum: Jedes Feld trägt ein **Blatt**, gezeichnet wie in jedem
   anderen Motiv dieser App — durch pen.repeat(), also in allen Feldern genau
   gleich. Das ist die Symmetrie, von der ein Mandala lebt; sie darf nicht
   davon abhängen, welche Zahl gerade in einem Feld steht. Zwischen den
   Blättern bleiben Zwickel, Ringe und Perlen zum Mitbemalen. Und **in** das
   Blatt kommt, klein und ruhig, die Menge oder die Aufgabe.

   Verschieden ist damit nur noch, was verschieden sein muss: die Zahl und –
   über verteile() – die Farbe. */

/* Das Blatt eines Feldes: von rInner bis rOuter, am breitesten genau dort,
   wo die Beschriftung steht. */
/* Wie viele Felder hat ein Band?

   Ein Mandala wird nach außen hin feiner, nicht gröber — der äußerste Ring
   trägt die meisten Glieder. Genau daran scheiterte der Entwurf mit überall
   gleich vielen Feldern: acht Felder sind innen richtig und außen viel zu
   wenige, und aus dem äußeren Ring wurden acht große Tafeln.

   Ein Band darf deshalb als drittes einen Faktor tragen: [r0, r1, 2] heißt
   doppelt so viele Felder wie die Grundachsenzahl. */
function kidsZahl(motif, band) {
  return motif.axes * (band[2] || 1);
}

/* Alle Bänder eines Blattes: die mit Aufgaben (bands) und die reinen
   Zierbänder (zier).

   Nicht jeder Ring muss eine Zahl tragen. Ein Rechenmandala, in dem jedes
   einzelne Feld eine Aufgabe hat, ist eine Übungsseite; eines mit einem
   Aufgabenring und zwei Zierringen ist ein Mandala, in dem gerechnet wird.
   Der Unterschied ist nicht nur hübsch: er hält die Zahl der Aufgaben bei
   zwanzig statt bei vierzig, und er lässt dem Kind Flächen, die es einfach
   so bemalen darf. */
function kidsBaender(motif) {
  return motif.bands.concat(motif.zier || []);
}

/* Die Form richtet sich nach dem Zuschnitt des Feldes, nicht umgekehrt.

   Ein Blütenblatt (petalPoints) läuft nach innen und außen spitz zu — schön,
   solange das Feld länger als breit ist. Im äußeren Ring ist es das nicht:
   dort ist ein Feld doppelt so breit wie hoch, und ein Blütenblatt darin wird
   zur rundlichen Beule, die aussieht, als sei sie danebengegangen. Für solche
   Felder ist die richtige Form die **Kartusche**: ein Band, das dem Ring
   folgt, mit abgerundeten Enden. Auch das ist ein Mandala-Baustein, nur eben
   der für liegende Felder. */
function kidsBlatt(band, step) {
  const einzug = Math.min(12, (band[1] - band[0]) * 0.10);
  const r0 = band[0] + einzug;
  const r1 = band[1] - einzug;
  const rm = (r0 + r1) / 2;
  const halb = step * 0.36;
  return { r0: r0, r1: r1, halb: halb, liegend: 2 * rm * halb > (r1 - r0) * 1.15 };
}

/* Der Umriss eines Feldes, um t einwärts geschrumpft (0 = voller Umriss). */
function kidsUmriss(b, t) {
  const spanne = b.r1 - b.r0;
  const r0 = b.r0 + spanne * t;
  const r1 = b.r1 - spanne * t;
  if (!b.liegend) return petalPoints(r0, r1, b.halb * (1 - t * 2.6), 32);
  return wedgeBandPoints(r0, r1, b.halb * (1 - t * 1.1), 20);
}

function kidsBuild(pen) {
  const motif = this;
  const ctx = pen.ctx;
  const step = pen.step;
  const rand = motif.rand === undefined ? KIDS_RAND : motif.rand;
  const alle = kidsBaender(motif).slice().sort(function (x, y) { return x[0] - y[0]; });
  const nabe = alle[0][0];

  /* Außenrand und Zierrand mit Perlenkranz. Der Zierrand bekommt seine
     eigenen kurzen Speichen: ohne sie ist er ein einziger, ringsum
     geschlossener Streifen, und ein Tipp hinein färbt zehn Prozent des
     Blattes auf einmal. Der Testlauf misst genau das. */
  drawRing(pen, R_OUT, 2.6);
  if (rand > 0) {
    const saum = makePen(ctx, kidsZahl(motif, alle[alle.length - 1]));
    drawRing(pen, R_OUT - rand, 2);
    ctx.lineWidth = 1.4;
    saum.repeat(function () {
      const a = UP + saum.step / 2;
      tracePath(ctx, [pol(R_OUT - rand, a), pol(R_OUT, a)], false);
    });
    drawDotAccent(saum, R_OUT - rand / 2, rand * 0.30, 1.6);
  }

  /* Die Ringe zwischen und um die Bänder – sie schließen die Zwickel, damit
     ein Tipp daneben nicht das halbe Blatt flutet. */
  alle.forEach(function (band, i) {
    if (i > 0) drawRing(pen, band[0], 1.6);
    if (band[1] < R_OUT - rand) drawRing(pen, band[1], 1.6);
  });

  /* Das Blatt je Feld, in jedem Feld dasselbe – das ist das Ornament, das ein
     Kind bemalt, und es steht unabhängig von jeder Zahl. Jedes Band bekommt
     seinen eigenen Stift, weil es seine eigene Feldzahl haben darf. */
  alle.forEach(function (band) {
    const feder = makePen(ctx, kidsZahl(motif, band));
    const step = feder.step;
    const b = kidsBlatt(band, step);
    const spanne = b.r1 - b.r0;

    /* Speichen zwischen den Feldern dieses Bandes, nicht durch sie hindurch. */
    ctx.lineWidth = 1.6;
    feder.repeat(function () {
      const a = UP + step / 2;
      tracePath(ctx, [pol(band[0], a), pol(band[1], a)], false);
    });

    drawClosedLoop(feder, kidsUmriss(b, 0), 2.2);
    drawClosedLoop(feder, kidsUmriss(b, 0.12), 1.3);

    /* Zwischen zwei Blättern bleibt ein Zwickel. Eine Perle darin füllt ihn,
       ohne mit der Beschriftung ins Gehege zu kommen — und gibt eine weitere
       kleine Fläche zum Bemalen. Sie sitzt auf der Speiche, also um einen
       halben Schritt gegen die Blätter versetzt. */
    const rm = (b.r0 + b.r1) / 2;
    /* Der Zwickel zwischen zwei Blättern ist (step − 2·halb)·rm breit; die
       Perle darf höchstens die Hälfte davon als Halbmesser haben, sonst
       schneidet sie in die Blätter. */
    const perle = Math.min(spanne * 0.13, rm * step * 0.10);
    ctx.lineWidth = 1.4;
    feder.repeat(function () {
      const c = pol(rm, UP + step / 2);
      ctx.beginPath();
      ctx.arc(c[0], c[1], perle, 0, TAU);
      ctx.stroke();
    });
  });

  /* Die Nabe: kein Rechenfeld, sondern der Lohn. */
  drawRing(pen, nabe, 2.2);
  drawClosedLoop(pen, petalPoints(nabe * 0.42, nabe - 8, step * 0.44), 1.8);
  if (nabe > 70) {
    drawRing(pen, nabe * 0.42, 1.8);
    drawClosedLoop(pen, petalPoints(6, nabe * 0.42 - 6, step * 0.46), 1.6);
  }
}

/* ---------------------------------------------------------------------------
   Zähl- und Rechenmandalas — die Verteilung

   Vorher wurden die Werte **gewürfelt** (shuffledFill). Wer alles richtig
   rechnete, bekam zufällig verteilte Farben: optisches Rauschen, kein Lohn.
   Bei einem guten Malen-nach-Zahlen ist die Verteilung komponiert — richtig
   gerechnet soll ein Muster entstehen.

   Die Aufgabe wird dadurch kein bisschen leichter. Es ändert sich nur, wohin
   die Anstrengung führt.

   Nebenbei fällt daraus die Geschwister-Idee: Felder mit demselben Ergebnis
   liegen jetzt symmetrisch zueinander, statt verstreut. Ein Kind kann
   entdecken, dass 3 + 4 und 5 + 2 zusammengehören — die Symmetrie zeigt die
   Mathematik, statt ihr im Weg zu stehen. */
const MUSTER = {
  /* Jeder v-te Platz dieselbe Farbe: ein Stern mit v Zacken je Ring. */
  stern: function (i, band, v) { return (i + band) % v; },
  /* Um eins versetzt je Ring: die Farben wandern nach außen. */
  wechsel: function (i, band, v, n) { return (i + band * 2) % v; },
  /* An der Senkrechten gespiegelt — ruhiger, weil es eine Achse gibt. */
  spiegel: function (i, band, v, n) {
    const j = Math.min(i, (n - i) % n);
    return (j + band) % v;
  }
};

function verteile(motif, cells) {
  const werte = motif.values;
  const v = werte.length;
  const regel = MUSTER[motif.muster] || MUSTER.stern;
  cells.forEach(function (cell) {
    cell.value = werte[regel(cell.i, cell.band, v, cell.n) % v];
  });
}
...
function mathTask(value, rng, motif) {
  ...
  const plus = function () {
    const a = 1 + Math.floor(rng() * Math.max(1, value - 1));
    return a + ' + ' + (value - a);
  };
  const minus = function () {
    const b = 1 + Math.floor(rng() * Math.min(Math.max(room, 1), 9));
    return (value + b) + ' − ' + b;
  };
  if (rng() < 0.55 && value >= 2) return plus();
  return room >= 1 ? minus() : plus();
}
...
function makeFields(motif) {
  ...
  /* Mitte und Platz eines Feldes kommen aus dem **Blatt**, nicht aus dem
     Tortenstück: Beschriftet wird die Fläche, die das Blatt einschließt, und
     die ist an ihrer breitesten Stelle immer noch schmaler als das Feld.
     halb ist der halbe Öffnungswinkel des Blattes, ht daraus die halbe Breite
     an der breitesten Stelle. */
  motif.bands.forEach(function (band, bandIndex) {
    const n = kidsZahl(motif, band);
    const step = TAU / n;
    const b = kidsBlatt(band, step);
    const mitte = (b.r0 + b.r1) / 2;
    for (let i = 0; i < n; i++) {
      cells.push({
        r: mitte,
        hr: (b.r1 - b.r0) * 0.30,
        ht: mitte * b.halb * 0.78,
        a: UP + i * step,
        band: bandIndex,
        i: i,
        n: n
  ...
  /* Komponiert, nicht gewürfelt — siehe verteile(). */
  if (motif.muster) verteile(motif, cells);
  else {
    const values = shuffledFill(motif.values, cells.length, rng);
    cells.forEach(function (cell, index) { cell.value = values[index]; });
  }

  /* Zählmandalas tragen kein label(): dort bleibt cell.text leer und an
     seiner Stelle steht die Menge als Punkte. */
  cells.forEach(function (cell) {
...
function makeLegend(motif, fields) {
  ...
  const eigen = state.legendFarben[motif.id] || {};
  return values.map(function (value, index) {
    return {
      value: value,
      hex: eigen[value] || pigments()[index % pigments().length].hex,
...
/* ---------------------------------------------------------------------------
   Die Legende gehört dem Kind

   Welche Farbe zu welcher Zahl gehört, ist keine mathematische Wahrheit. Wer
   Rot für die Fünf will, soll Rot für die Fünf haben – ein Tipp auf den
   kleinen Pfeil neben einem Legendeneintrag holt das nächste freie Pigment.
   Bereits gefüllte Felder behalten ihre Farbe; geändert wird nur, womit ab
   jetzt gemalt wird. Die Wahl bleibt am Motiv hängen und überlebt das
   Schließen der App. */
function tauscheLegende(value) {
  if (!state.motif || !state.legend.length) return;
  const alle = pigments().map(function (p) { return p.hex; });
  const belegt = state.legend
    .filter(function (i) { return i.value !== value; })
    .map(function (i) { return i.hex; });
  const jetzt = (state.legend.filter(function (i) { return i.value === value; })[0] || {}).hex;

  let start = alle.indexOf(jetzt);
  if (start < 0) start = -1;
  let neu = null;
  for (let k = 1; k <= alle.length; k++) {
    const kandidat = alle[(start + k + alle.length) % alle.length];
    if (belegt.indexOf(kandidat) < 0) { neu = kandidat; break; }
  }
  if (!neu) neu = alle[(start + 1) % alle.length];

  const eigen = state.legendFarben[state.motif.id] || {};
  eigen[value] = neu;
  state.legendFarben[state.motif.id] = eigen;
  Store.set('legendFarben', state.legendFarben);

  state.legend = makeLegend(state.motif, state.fields);
  renderLegend();
  buildQuickLegend();
  state.color = neu;
...
function renderLabels() {
  ...
  state.fields.forEach(function (cell) {
    ...
    const raum = feldRaum(cell);

    if (cell.text) {
      /* Von oben herunter, bis die Aufgabe wirklich ins Blatt passt. */
      let size = 26;
      ctx.font = labelFont(size);
      while (size > 9) {
        const w = ctx.measureText(cell.text).width;
        if (passtEin(w, size * 0.78, raum) === 1) break;
        size -= 1;
        ctx.font = labelFont(size);
      }
      ctx.fillText(cell.text, c[0], c[1]);
    } else {
      zeichneMenge(ctx, c[0], c[1], cell.value, raum);
    }
  });
...
/* ---------------------------------------------------------------------------
   Wieviel Platz hat ein Feld — waagerecht und senkrecht?

   Beschriftungen stehen **aufrecht**, das Blatt aber liegt schräg. Der erste
   Entwurf maß die Breite als Sehne des Ringes und die Höhe als halbe
   Bandbreite: auf zwölf Uhr stimmt das, auf neun Uhr steht es genau falsch
   herum. Dort ragte die Beschriftung über die Speiche und über den Rand.

   Deshalb wird das Blatt als Ellipse genähert — Halbachse hr nach außen,
   Halbachse ht quer dazu — und daraus der waagerechte und senkrechte
   Halbmesser im Bildschirmmaß gerechnet. Ein aufrechtes Rechteck darin
   erfüllt (w/A)² + (h/B)² ≤ 1; hr und ht sind bereits konservativ gewählt,
   deshalb reicht hier ein kleiner Abstand zur Linie. */
function feldRaum(cell) {
  const ca = Math.abs(Math.cos(cell.a));
  const sa = Math.abs(Math.sin(cell.a));
  return {
    a: Math.hypot(cell.hr * ca, cell.ht * sa),   // waagerechter Halbmesser
    b: Math.hypot(cell.hr * sa, cell.ht * ca)    // senkrechter Halbmesser
  };
}

/* Passt ein aufrechtes Rechteck w × h in die Ellipse? Die Bedingung lautet
   (w/2A)² + (h/2B)² ≤ ziel. Zurück kommt der Faktor, mit dem w und h zu
   verkleinern sind — bei 1 passt es bereits.

   Ohne diese Prüfung genügte es, dass w ≤ 2A und h ≤ 2B war. Das erlaubt ein
   Rechteck, dessen **Ecken** weit außerhalb liegen: bei einer Rechenaufgabe
   auf drei Uhr stand die Aufgabe dann quer über beide Blattspitzen hinaus. */
function passtEin(w, h, raum, ziel) {
  const t = Math.sqrt(Math.pow(w / (2 * raum.a), 2) + Math.pow(h / (2 * raum.b), 2));
  const z = Math.sqrt(ziel === undefined ? 0.80 : ziel);
  return t <= z ? 1 : z / t;
}
...
/* ---------------------------------------------------------------------------
   ...
   Ein Vorschulkind liest keine Ziffer, es sieht eine Menge. Deshalb steht in
   einem Zählfeld nicht die Zahl, sondern die Menge selbst – als Punkte in
   Fünferreihen oder als Fünferstrich.

   Zwei weitere Versuche sind wieder heraus, und aus demselben Grund: das
   Würfelbild und das Zehnerfeld. Beide brauchen einen **Kasten**, und ein
   Kasten in jedem Feld erschlägt das Mandala – aus der Vorlage wird ein
   Spielbrett. Was aus dem Feld nichts Fremdes macht, ist die Menge ohne
   Rahmen: Punkte und Striche. Dabei bleibt es.

   Beide zeichnen in dasselbe Rechteck (room × height) und richten sich selbst
   nach der Feldgröße. Welche gewählt wird, steht am Motiv (zeichen). */
function zeichneMenge(ctx, x, y, count, raum) {
  /* Gezeichnet wird breiter als hoch. Liegt das Blatt hochkant — auf drei und
     auf neun Uhr —, wird einmal gedreht statt gesondert gerechnet. */
  if (raum.b > raum.a) {
    ctx.save();
    ctx.translate(x, y);
    ctx.rotate(-Math.PI / 2);
    zeichnePunkte(ctx, 0, 0, count, { a: raum.b, b: raum.a });
    ctx.restore();
    return;
  }
  zeichnePunkte(ctx, x, y, count, raum);
}

/* Punkte in Fünferreihen – so bleibt die Menge auf einen Blick erfassbar.
   Bis vier eine Reihe, sechs als zwei Dreier, darüber Fünferreihen. */
function zeichnePunkte(ctx, x, y, count, raum) {
  const perRow = count <= 4 ? count : (count === 6 ? 3 : 5);
  const rows   = Math.ceil(count / perRow);

  /* Erst großzügig rechnen, dann so weit verkleinern, dass die Ecken der
     Punktgruppe innerhalb des Blattes bleiben. */
  let gap = Math.min(30, 2 * raum.a / perRow, 2 * raum.b / rows);
  gap *= passtEin(gap * perRow, gap * rows, raum, 0.72);
  const radius = Math.max(2, gap * 0.33);
...
const Gallery = {
  ...
  open: function () {
    ...
    return new Promise(function (resolve) {
      ...
      let request;
      try {
        request = indexedDB.open('mandala-atelier-regal2-21', 1);
      } catch (err) {
        return resolve();
      }
...
function renderLegend() {
  ...
  state.legend.forEach(function (item) {
    ...
    /* Farbe neu vergeben – siehe tauscheLegende(). */
    const tausch = document.createElement('button');
    tausch.type = 'button';
    tausch.className = 'legend-tausch';
    tausch.dataset.value = String(item.value);
    tausch.textContent = '↻';
    tausch.title = 'Andere Farbe für ' + item.text;
    tausch.setAttribute('aria-label', 'Andere Farbe für ' + item.text);
    li.appendChild(tausch);
...
function bindEvents() {
  ...
  document.addEventListener('click', function (event) {
    ...
    const tausch = target.closest('.legend-tausch');
    if (tausch) { tauscheLegende(Number(tausch.dataset.value)); return; }
...
function start() {
  ...
  const eigeneLegende = Store.get('legendFarben', {});
  if (eigeneLegende && typeof eigeneLegende === 'object') state.legendFarben = eigeneLegende;
  buildPalette();
