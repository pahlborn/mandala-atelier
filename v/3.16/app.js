...
/* Die Farbwelten des Mandala Ateliers, Ton für Ton dieselben – ihre Stimmung
   ...
   Zehn bis vierzehn je Welt, und die Zahl ist gemessen, nicht gesetzt: Hier
   wird Farbe **durchgerieben**, nicht deckend aufgetragen. Bei 70 % Deckung
   auf dem Papierton schrumpft jeder Abstand auf etwa sieben Zehntel, und was
   in der Leiste noch zwei Töne sind, ist auf dem Blatt einer. Deshalb liegt
   in keiner Welt mehr ein Paar unter ΔE00 7 – bei 70 % Deckung gerechnet,
   nicht am reinen Pigment.
...
function nextThought() {
  let seen = [];
  try { seen = JSON.parse(localStorage.getItem('atelier3-regal3-16-gedanken') || '[]'); } catch (err) {}
  ...
  try { localStorage.setItem('atelier3-regal3-16-gedanken', JSON.stringify(seen)); } catch (err) {}
...
const Speicher = {
  ...
  open: function () {
    ...
    return new Promise(function (resolve) {
      let request;
      try {
        request = indexedDB.open('atelier3-regal3-16', 1);
      } catch (err) {
        resolve(false);
        return;
      }
...
/* Tag oder Nacht ist keine Einstellung – es sind zwei Papiere. Auf dem
   ...
   Papiere selbst. */
const PAPER_KEY = 'atelier3-regal3-16-papier';
...
function bindUi() {
  ...
  ui.traySound.addEventListener('click', function () {
    ...
    try { localStorage.setItem('atelier3-regal3-16-ton', Klang.muted ? 'aus' : 'an'); } catch (err) {}
    syncSoundLabel();
  });
...
async function start() {
  ...
  try { Klang.muted = localStorage.getItem('atelier3-regal3-16-ton') === 'aus'; } catch (err) {}
  syncSoundLabel();
  ...
  let seen = false;
  try { seen = localStorage.getItem('atelier3-regal3-16-gesehen') === 'ja'; } catch (err) {}
  if (!seen && !sheet.touched) {
    showHint('Streiche über das Blatt.', 9000);
    try { localStorage.setItem('atelier3-regal3-16-gesehen', 'ja'); } catch (err) {}
  }
