...
/* Fünf Farbwelten zu **je zehn** Pigmenten. Alle gedeckt – keine Buntstift-
   Knallfarben; das trägt die erwachsene Anmutung wesentlich.

   Zehn, und in jeder Welt gleich viele: Eine kürzere Tafel neben einer
   längeren sieht nach Versehen aus, und zehn ist zugleich die Untergrenze —
   „Zähl bis 10" vergibt zehn Farben der Reihe nach und braucht sie
   unterscheidbar.

   Welche zehn, ist **gerechnet**, nicht ausgesucht. Aus den vierzehn jeder
   Welt wurde die Teilmenge von zehn gewählt, deren engstes Paar am weitesten
   auseinanderliegt — CIEDE2000, bei 70 % Deckung auf dem Papierton, und in
   **beiden** Modi von Blatt.

   Beide Modi sind der Kern der Sache. Bei Tag liegt Pigment auf hellem
   Papier; bei Nacht rechnet Blatt jeden Ton in Kreide um, und dort entschied
   früher die Umrechnung über die Helligkeit statt das Pigment. Das ebnete
   genau den Unterschied ein, von dem zwei gedeckte Töne leben: Rauch und
   Asche standen als Kreide bei **ΔE 0.0**, Ocker und Gold ebenso. Bei Tag
   lagen dieselben Paare zweistellig auseinander. Wer nur bei Tag misst, sieht
   davon nichts.

   Repariert ist beides: asChalk() bildet die Helligkeit jetzt ab, statt sie
   zu setzen (siehe atelier3/app.js), und die Auswahl misst in beiden Modi.
   Das engste Paar liegt seither bei ΔE 5.1 statt bei 0.0. `npm test` misst
   es bei jedem Lauf und nennt den Modus, in dem es eng wird. */
const PALETTES = [
  {
  ...
  /* Goldgrund – als einzige Welt keine Sammlung von Tönen, sondern **Leitern**.
     ...
     Dafür braucht es Reihen desselben Tons, nicht zehn verschiedene. Hier
     stehen drei Leitern zu je drei Stufen – Gold, Indigo, Purpur –, dazu der
     Grund als dunkles Ende. Ursprünglich hatte Gold fünf Stufen und Indigo
     vier; die Zwischenstufen sind der Auslese zum Opfer gefallen, weil sie
     als Kreide mit ihren Nachbarn zusammenfielen (Ocker/Gold: ΔE 0.0). Drei
     Stufen genügen für einen Verlauf, und sie halten bei Tag wie bei Nacht.
...
const Store = {
  prefix: 'mandala-atelier-regal2-36.',
  get(key, fallback) {
    try {
...
const MOTIFS = [
  ...
  /* --- Geometrisch-klassisch ------------------------------------------- */
  {
    ...
    name: 'Sternkranz', note: 'Zwölf Spitzen, ruhiger Grundriss', mythos: 'Zwölf Spitzen im Kreis — als würde ein einzelner Stern sich selbst vervielfachen, bis er den ganzen Rand erreicht.',
    build: function (p) {
      drawRing(p, 352, 1.8);
      drawRing(p, 288, 1.8);
      drawRing(p, 158, 1.8);
  ...
  },
  {
    ...
    name: 'Rautenkranz', note: 'Rauten in drei Größen', mythos: 'Rauten in drei Größen, ineinandergestaffelt — wie ferne Kirchenfenster, die sich zu einem einzigen Muster ordnen.',
    build: function (p) {
      drawRing(p, 330, 1.8);
      drawRing(p, 210, 1.8);
  ...
  },
  {
    ...
    name: 'Sternmandala fein', note: 'Sechzehn Achsen, viele kleine Felder', mythos: 'Sechzehn Achsen, unzählige kleine Felder — je genauer man hinsieht, desto mehr öffnet sich.',
    build: function (p) {
      drawRing(p, 368, 1.4);
      drawRing(p, 300, 1.4);
      drawRing(p, 226, 1.4);
...

  {
    ...
    name: 'Achteckstern', note: 'Acht Achsen, klare Kanten', mythos: 'Acht Spitzen, scharf und gleichmäßig — die Form, die entsteht, wenn zwei Quadrate sich umeinander drehen.',
    build: function (p) {
      drawRing(p, 366, 1.6);
      drawRing(p, 296, 1.8);
      drawRing(p, 226, 1.6);
  ...
  },
  {
    ...
    name: 'Gitterrose', note: 'Verschränkte Rauten, dichtes Netz', mythos: 'Ein Gitterwerk, das sich zur Rose rundet — Ordnung, die von selbst zu blühen beginnt.',
    build: function (p) {
      drawRing(p, 372, 1.4);
      drawRing(p, 308, 1.6);
      drawRing(p, 244, 1.6);
  ...
  /* --- Natur ------------------------------------------------------------ */
  {
    ...
    name: 'Blüte', note: 'Acht große Blätter, viel Fläche', mythos: 'Blütenblatt um Blütenblatt, wie sie sich in einem einzigen warmen Moment öffnen.',
    build: function (p) {
      drawRing(p, 122, 2);
      drawRing(p, 214, 1.4);
      drawRing(p, 306, 1.4);
  ...
  },
  {
    ...
    name: 'Blätterkranz', note: 'Blätter mit Mittelrippe, versetzt', mythos: 'Ein Kranz aus Blättern, so dicht wie ein Wald im Hochsommer.',
    build: function (p) {
      drawRing(p, 300, 1.8);
      drawRing(p, 140, 1.8);
  ...
  },
  {
    ...
    name: 'Muschelspirale', note: 'Sechs Arme, weite Bögen', mythos: 'Eine Spirale, wie sie das Meer in jede Muschel schreibt, bevor sie an den Strand gespült wird.',
    build: function (p) {
      drawRing(p, 360, 1.6);
      drawRing(p, 290, 1.6);
      drawRing(p, 220, 1.6);
...

  {
    ...
    name: 'Farnkreis', note: 'Wedel mit feinen Fiedern', mythos: 'Wedel, die sich entrollen wie ein Farn im ersten Frühlingslicht.',
    build: function (p) {
      drawRing(p, 360, 1.4);
      drawRing(p, 292, 1.6);
      drawRing(p, 220, 1.6);
  ...
  },
  {
    ...
    name: 'Samenkranz', note: 'Sechzehn Samen, feine Teilung', mythos: 'Unzählige kleine Samen, dicht an dicht — ein ganzer Sommer, der auf den nächsten wartet.',
    build: function (p) {
      drawRing(p, 374, 1.3);
      drawRing(p, 312, 1.4);
      drawRing(p, 252, 1.4);
  ...
  /* --- Zen & Achtsamkeit ------------------------------------------------ */
  {
    ...
    name: 'Wellenkreis', note: 'Fünf Wellenringe, gleichmäßiger Takt', mythos: 'Wellen, die sich immer wieder überlagern, ohne sich je ganz zu wiederholen.',
    build: function (p) {
  ...
  },
  {
    ...
    name: 'Tropfenkranz', note: 'Tropfen in zwei Lagen, versetzt', mythos: 'Tropfen, aufgereiht wie der Klang von Wasser, das ganz langsam fällt.',
    build: function (p) {
      drawRing(p, 322, 1.8);
      drawRing(p, 168, 1.8);
  ...
  },
  {
    ...
    name: 'Ruhefeld', note: 'Wenige große Flächen, viel Raum', mythos: 'Ein weites, stilles Feld — Platz genug, dass der Blick zur Ruhe kommt, bevor er weiterwandert.',
    build: function (p) {
      drawRing(p, 388, 1.6);
      drawRing(p, 356, 1.6);
      drawRing(p, 250, 1.6);
...

  {
    ...
    name: 'Atemringe', note: 'Ruhiger Takt, gleichmäßige Weite', mythos: 'Ringe, die sich weiten und wieder schließen — wie Atem, der kommt und geht, ohne dass man ihn zählen müsste.',
    build: function (p) {
  ...
  },
  {
    ...
    name: 'Steingarten', note: 'Wenige Formen, geharkte Bahnen', mythos: 'Wenige Formen, weit auseinandergesetzt — wie Steine, die in Sand geharkt wurden, nicht wie sie zufällig liegen.',
    build: function (p) {
  ...
  /* --- Jahreszeiten ----------------------------------------------------- */
  {
    ...
    name: 'Winter', note: 'Schneekristall mit Seitenästen', mythos: 'Kahle, klare Linien — die Ruhe, die jedem neuen Anfang vorausgeht.',
    build: function (p) {
      drawRing(p, 372, 1.4);
      drawRing(p, 300, 1.4);
      drawRing(p, 224, 1.4);
  ...
  },
  {
    ...
    name: 'Frühling', note: 'Knospen in drei Lagen', mythos: 'Erste zarte Formen, die sich gerade erst trauen, sich zu öffnen.',
    build: function (p) {
      drawRing(p, 356, 1.4);
      drawRing(p, 300, 1.6);
      drawRing(p, 226, 1.4);
  ...
  },
  {
    ...
    name: 'Sommer', note: 'Strahlenkranz um eine offene Mitte', mythos: 'Volle, satte Formen — die Jahreszeit, in der alles zugleich blüht.',
    build: function (p) {
      drawRing(p, 340, 1.8);
      drawRing(p, 196, 2);
      drawRing(p, 110, 2);
  ...
  },
  {
    ...
    name: 'Herbst', note: 'Geneigte Blätter, Eicheln als Punkte', mythos: 'Formen, die sich langsam neigen, bevor sie zu Boden sinken — der ruhigste aller Übergänge.',
    build: function (p) {
      drawRing(p, 318, 1.8);
      drawRing(p, 176, 1.8);
     ...
     Raum, und ein Tipp färbt den halben Palast. */
  {
    ...
    name: 'Anlage', note: 'Vier Tore, drei Bereiche', mythos: 'Ein Grundriss, wie ihn alte Gärten tragen — eine Ordnung, in die man gedanklich hineingehen kann.',
    zones: [
     ...
     sondern dem Rhythmus des Bandes, in dem man steht. */
  {
    ...
    name: 'Ringanlage', note: 'Vier Tore, vier Ringbänder', mythos: 'Ring um Ring um einen stillen Mittelpunkt — wie eine Stadt, die sich um ihr eigenes Zentrum legt.',
    zones: [
...

  {
    ...
    name: 'Gartenanlage', note: 'Vier Wasserläufe, sechsunddreißig Beete', mythos: 'Beete und Wasserläufe, symmetrisch angelegt — ein Garten, durch den man mit den Augen spazieren geht.',
    zones: [
...

  {
    ...
    name: 'Sternanlage', note: 'Acht Bastionen, eine Piazza', mythos: 'Ein Grundriss, der von oben wie ein Stern aussieht — Verteidigung, die zur Form wurde.',
    zones: [
...

  {
    ...
    name: 'Rasteranlage', note: 'Neun mal neun Felder, kein Kranz', mythos: 'Ein Raster aus geraden Wegen — die stille Ordnung, die jede große Anlage im Kern trägt.',
    zones: [
...

  {
    ...
    name: 'Stufenanlage', note: 'Außen eckig, innen rund', mythos: 'Stufe um Stufe, die sich nach innen wandelt — wie ein Bauwerk, das man erst umrunden muss, um es ganz zu verstehen.',
    zones: [
...

  {
    ...
    name: 'Torstadt', note: 'Zwölf Tore, sechs Mauern', mythos: 'Tore an allen Seiten — eine Stadt, die von jeder Richtung aus betreten werden kann.',
    zones: [
     ...
     Kassetten. */
  {
    ...
    name: 'Kuppelanlage', note: 'Ein Gewölbe von unten, 132 Kassetten', mythos: 'Eine Wölbung, die sich Kassette für Kassette dem Licht entgegenhebt.',
    zones: [
     ...
     wird sie von Eltern und Lehrkräften – deshalb kein Kinder-Look. */
  {
    ...
    name: 'Erste Formen', note: 'Kindergarten – sehr große Felder', mythos: 'Die einfachsten Formen zuerst — jede große Vorlage hier hat mit genau solchen angefangen.',
    build: function (p) {
      drawRing(p, 300, 3);
      drawRing(p, 150, 3);
  ...
  },
  {
    ...
    name: 'Mustertanz', note: 'Grundschule – Bänder im Wechsel', mythos: 'Formen, die sich abwechseln, als würden sie im Kreis tanzen.',
    build: function (p) {
      drawRing(p, 330, 2);
      drawRing(p, 190, 2);
  ...
  },
  {
    ...
    name: 'Formenreigen', note: 'Kindergarten – runde und eckige Felder', mythos: 'Ein ganzer Reigen kleiner Formen — jede für sich einfach, gemeinsam ein Fest.',
    build: function (p) {
      drawRing(p, 320, 2.6);
      drawRing(p, 220, 2.6);
      drawRing(p, 130, 2.6);
...
/* ===== FEINWERK – ANFANG ====================================================
   ...
   Alles zwischen diesem Zeichen und „FEINWERK – ENDE“ ist zusätzlich und
   hängt an nichts. Es ändert keine Zeile über sich: die Welt und die Motive
   werden an WORLDS und MOTIFS angehängt statt in sie hineingeschrieben, und
   die fünf Bausteine unten sind neue Funktionen, keine geänderten.

   Damit ist der Bereich in einem Stück entfernbar. Was dann passiert, ist
   nachgesehen und nicht vermutet: loadMotif() mit einer unbekannten Kennung
   liefert null und zeigt ein leeres Blatt; gesicherte Werke liegen in der
   Galerie als Bild, nicht als Bauanweisung, und überleben das Entfernen
   unverändert. Die Fassung davor bleibt außerdem unter v/ stehen.

   Vier der zwölf stehen nicht in Feinwerk, sondern in Geometrisch-klassisch
   und Natur: Kordelstern und Gitterschale, weil der Stern und das Gitter
   alles andere schlagen; Tropfensaum und Rankengeflecht, weil Tropfen und
   Ranken organisch lesen. Sie werden trotzdem von hier aus angehängt und
   nicht in die Listen oben geschrieben - sonst wäre der Bereich nicht mehr
   in einem Stück entfernbar, und die bestehenden Welten stünden nach dem
   Entfernen nicht wieder bei fünf.

   Die Regel für die Zuordnung: Lässt sich benennen, was zu sehen ist, mit
   einem Wort für ein Ding in der Welt - Blatt, Blume, Stern, Gitter? Dann
   dorthin. Lautet die einzige ehrliche Antwort „so ist es gemacht"? Dann
   Feinwerk.

   Worum es geht: Dichte. Die 34 Motive darüber zeichnen sieben bis zwölf
   Formen; die Vorbilder aus dem Handel vierzig bis sechzig. Dafür fehlten
   genau drei Bausteine, und alle drei sind reine Geometrie – Innenkontur,
   Sprossen, Perlreihe. Herkunft und Begründung je Motiv:
   docs/feinwerk.html.
   ------------------------------------------------------------------------ */

/* Dieselbe Form noch einmal, um ihren eigenen Schwerpunkt verkleinert.
   Aus einem Blatt wird ein Blatt mit Rand, aus einem Feld werden zwei. Das
   ist der billigste Weg zu mehr Feldern, der die Zeichnung nicht zerlegt,
   sondern vertieft. */
function konturPoints(points, faktor) {
  let sx = 0, sy = 0;
  for (let i = 0; i < points.length; i++) { sx += points[i][0]; sy += points[i][1]; }
  const cx = sx / points.length, cy = sy / points.length;
  return points.map(function (p) {
    return [cx + (p[0] - cx) * faktor, cy + (p[1] - cy) * faktor];
  });
}

/* Ein Bogenstück innerhalb eines Segments – die Querlinie zu den Sprossen. */
function bogenPoints(r, halfAngle, steps) {
  steps = steps || 16;
  const pts = [];
  for (let i = 0; i <= steps; i++) {
    pts.push(pol(r, UP - halfAngle + 2 * halfAngle * (i / steps)));
  }
  return pts;
}

/* Spitz nach außen, breit innen – der Strahl. */
function trianglePoints(rInner, rOuter, halfAngle) {
  return [pol(rOuter, UP), pol(rInner, UP - halfAngle), pol(rInner, UP + halfAngle)];
}

/* Eine Ranke: läuft nach außen und weicht dabei zur Seite aus und zurück. */
function vinePoints(rInner, rOuter, curl, steps) {
  steps = steps || 60;
  const pts = [];
  for (let i = 0; i <= steps; i++) {
    const t = i / steps;
    pts.push(pol(rInner + (rOuter - rInner) * t, UP + curl * Math.sin(Math.PI * 1.4 * t)));
  }
  return pts;
}

/* Sprossen: radiale Striche quer durch ein Band. Aus einem Ring werden
   Zellen, ohne dass eine neue Form nötig wäre. */
function drawSprossen(pen, rInner, rOuter, halfAngle, count, lineWidth, phase) {
  const ctx = pen.ctx;
  ctx.lineWidth = lineWidth || 1;
  phase = phase || 0;
  for (let k = 0; k < count; k++) {
    const a = phase + (count === 1 ? 0 : -halfAngle + 2 * halfAngle * (k / (count - 1)));
    const pts = [pol(rInner, UP + a), pol(rOuter, UP + a)];
    pen.repeat(function () { tracePath(ctx, pts, false); });
  }
}

/* Perlreihe: Kügelchen auf einem Bogen – die Granulation der Goldschmiede. */
function drawPerlen(pen, r, halfAngle, count, radius, lineWidth, phase) {
  const ctx = pen.ctx;
...
/* Eine Form auf der halben Teilung – zwischen den Formen der vollen. */
function halb(pen, points) { return rotatePoints(points, pen.step / 2); }

/* Auslöschung: Was hier gezeichnet wird, nimmt Farbe WEG statt sie
   aufzutragen. Erst damit kann eine Linie eine andere unterbrechen — und
   erst dadurch geht ein Strang sichtbar UNTER einem anderen hindurch.

   Für das Füllwerkzeug ist das der einzig richtige Weg. Ein deckender
   heller Balken bliebe eine Wand: blockedMask() prüft den Alphawert, nicht
   die Farbe, und ein weiß gemalter Streifen hat vollen Alphawert.
   'destination-out' setzt ihn auf 0 — die Stelle ist danach keine Wand
   mehr und bleibt ausmalbar. */
function radieren(pen, breite, zeichne) {
  const ctx = pen.ctx;
  ctx.save();
...
/* Ein Stück eines Wellenrings – für das Nachziehen nach dem Radieren. */
function wavyArcPoints(base, amplitude, lobes, a0, a1, steps) {
  steps = steps || 30;
  const pts = [];
  for (let i = 0; i <= steps; i++) {
...
/* Ein echtes Flechtband: zwei gegenläufige Wellen, die einander abwechselnd
   über- und unterqueren. Die Kreuzungen liegen dort, wo cos(lobes·a) null
   wird – also fest berechenbar und nicht gesucht. An jeder zweiten wird der
   obenliegende Strang breit ausradiert und danach nachgezogen; was darunter
   lag, ist dann unterbrochen. */
function drawFlechtband(pen, base, amp, lobes, halb2, lineWidth) {
  const ctx = pen.ctx;
  const kanten = function (vz) {
    return [wavyRingPoints(base + halb2, amp * vz, lobes),
            wavyRingPoints(base - halb2, amp * vz, lobes)];
  };
  ctx.lineWidth = lineWidth;
  [1, -1].forEach(function (vz) {
    kanten(vz).forEach(function (p) { tracePath(ctx, p, true); });
  });

  const schritt = Math.PI / lobes;
  for (let k = 0; k < 2 * lobes; k++) {
    const mitte = (Math.PI / 2 + k * Math.PI) / lobes;
    const vz = (k % 2 === 0) ? 1 : -1;          // wer hier oben liegt
    /* Schmal radieren. Bei ±0,30 Teilungen verlor jeder Strang dreißig
       Prozent seiner Länge und das Band zerfiel in Kommas; ±0,11 nimmt nur
       weg, was wirklich unter dem anderen liegt. */
    const a0 = mitte - schritt * 0.11, a1 = mitte + schritt * 0.11;
    radieren(pen, 2 * halb2 + lineWidth * 1.6, function () {
      tracePath(ctx, wavyArcPoints(base, amp * vz, lobes, a0, a1, 16), false);
    });
    ctx.lineWidth = lineWidth;
    [halb2, -halb2].forEach(function (off) {
      tracePath(ctx, wavyArcPoints(base + off, amp * vz, lobes,
                                   a0 - schritt * 0.04, a1 + schritt * 0.04, 20), false);
    });
  }
}

/* Oval um den Punkt (rMid, UP) - das Glied einer Kette. */
function ovalPoints(rMid, laenge, breite, steps) {
  steps = steps || 30;
  const pts = [];
  for (let i = 0; i < steps; i++) {
    const t = TAU * (i / steps);
    pts.push(pol(rMid + laenge * Math.cos(t), UP + breite * Math.sin(t)));
  }
  return pts;
}

/* Eine Schuppe: außen ein Bauch, innen die Sehne. Reihenweise versetzt
   gelegt, deckt jede die Naht der darunterliegenden. */
function schuppePoints(rInner, rOuter, halfAngle, steps) {
  steps = steps || 18;
  const pts = [];
  for (let i = 0; i <= steps; i++) {
    const t = i / steps;
    pts.push(pol(rInner + (rOuter - rInner) * Math.sin(Math.PI * t),
                 UP - halfAngle + 2 * halfAngle * t));
  }
  for (let i = steps; i >= 0; i--) {
    const t = i / steps;
    pts.push(pol(rInner, UP - halfAngle + 2 * halfAngle * t));
  }
  return pts;
}

/* Zellenschmelz: ein Band, in count Zellen geteilt, jede mit eigenem Grund.
   Der Goldschmied lötet die Wände und füllt danach - genau die Reihenfolge,
   in der auch hier gemalt wird. */
function drawZellenband(pen, rInner, rOuter, halfAngle, count, lineWidth, phase) {
  const breite = 2 * halfAngle / count;
  const zelle = wedgeBandPoints(rInner, rOuter, breite / 2, 6);
  const innen = konturPoints(zelle, 0.6);
  for (let k = 0; k < count; k++) {
    const a = (phase || 0) - halfAngle + breite / 2 + k * breite;
    drawClosedLoop(pen, rotatePoints(zelle, a), lineWidth || 1.3);
    drawClosedLoop(pen, rotatePoints(innen, a), (lineWidth || 1.3) * 0.7);
  }
}

WORLDS.push({ id: 'feinwerk', title: 'Feinwerk' });

MOTIFS.push(
  {
    id: 'spitzenrad', world: 'feinwerk', axes: 16,
    name: 'Spitzenrad', note: 'Vier Bänder mit Sprossen, Klöppelspitze als Rad',
    mythos: 'Spitze entsteht, indem man Fäden umeinanderführt und den Zwischenraum stehen lässt. Sie ist die einzige Handarbeit, deren Ergebnis hauptsächlich aus dem besteht, was nicht getan wurde.',
    build: function (p) {
      drawRing(p, 380, 1.2);
      drawRing(p, 310, 1.2);
      drawRing(p, 228, 1.2);
      drawRing(p, 146, 1.2);
      drawSprossen(p, 146, 228, p.step * 0.28, 3, 1);
      drawSprossen(p, 228, 310, p.step * 0.28, 3, 1);
      drawSprossen(p, 310, 380, p.step * 0.28, 3, 1);
      drawClosedLoop(p, halb(p, petalPoints(148, 226, p.step * 0.229, 14, 0.8)), 1.4);
      drawClosedLoop(p, petalPoints(230, 308, p.step * 0.229, 14, 0.8), 1.4);
      drawClosedLoop(p, halb(p, petalPoints(312, 378, p.step * 0.204, 12, 0.8)), 1.4);
      drawClosedLoop(p, petalPoints(382, 408, p.step * 0.153, 10, 0.8), 1.2);
      drawPerlen(p, 100, 0, 1, 11, 1.3);
      drawClosedLoop(p, petalPoints(14, 44, p.step * 1.07, 10, 0.8), 1.4);
    }
  },
  {
    id: 'perlgrat', world: 'feinwerk', axes: 16,
    name: 'Perlgrat', note: 'Schmale Blätter mit Innenkontur, geteilt durch einen Perlgrat',
    mythos: 'Der Grat ist die Stelle, an der zwei Hälften sich einigen. Man sieht ihn nur, wenn man das Blatt gegen das Licht dreht — und dann sieht man nichts anderes mehr.',
    build: function (p) {
      drawRing(p, 326, 1.2);
      drawRing(p, 232, 1.2);
      drawRing(p, 130, 1.2);
      drawClosedLoop(p, petalPoints(132, 326, p.step * 0.331, 20, 0.9), 1.7);
      drawClosedLoop(p, konturPoints(petalPoints(132, 326, p.step * 0.331, 20, 0.9), 0.55), 1);
      drawClosedLoop(p, halb(p, petalPoints(330, 406, p.step * 0.255, 12, 0.8)), 1.4);
      drawClosedLoop(p, halb(p, petalPoints(58, 128, p.step * 0.306, 12, 0.8)), 1.3);
      drawSprossen(p, 232, 326, p.step * 0.255, 3, 0.85);
      drawPerlen(p, 180, p.step * 0.153, 2, 7, 1.1, p.step / 2);
      drawPerlen(p, 368, p.step * 0.127, 2, 6, 1.1);
      drawClosedLoop(p, petalPoints(14, 42, p.step * 1.02, 10, 0.8), 1.4);
    }
  },
  {
    id: 'tropfensaum', world: 'natur', axes: 12,
    name: 'Tropfensaum', note: 'Vier Reihen Tropfen mit Innen- und Kernkontur',
    mythos: 'Ein Tropfen hat keine Kanten, und trotzdem weiß jeder sofort, wo er aufhört. Es gibt Dinge, die brauchen keine Linie, um eine Grenze zu haben.',
    build: function (p) {
      drawRing(p, 310, 1);
      drawRing(p, 214, 1);
      drawClosedLoop(p, petalPoints(60, 168, p.step * 0.458, 16, 0.5), 1.6);
      drawClosedLoop(p, konturPoints(petalPoints(60, 168, p.step * 0.458, 16, 0.5), 0.55), 0.95);
      drawClosedLoop(p, halb(p, petalPoints(160, 268, p.step * 0.42, 16, 0.5)), 1.6);
      drawClosedLoop(p, halb(p, konturPoints(petalPoints(160, 268, p.step * 0.42, 16, 0.5), 0.55)), 0.95);
      drawClosedLoop(p, petalPoints(260, 352, p.step * 0.382, 16, 0.5), 1.6);
      drawClosedLoop(p, konturPoints(petalPoints(260, 352, p.step * 0.382, 16, 0.5), 0.55), 0.95);
      drawClosedLoop(p, halb(p, petalPoints(344, 408, p.step * 0.325, 14, 0.5)), 1.6);
      drawClosedLoop(p, halb(p, konturPoints(petalPoints(344, 408, p.step * 0.325, 14, 0.5), 0.55)), 0.95);
      drawClosedLoop(p, halb(p, petalPoints(62, 166, p.step * 0.21, 14, 0.6)), 1);
      drawClosedLoop(p, petalPoints(162, 266, p.step * 0.191, 14, 0.6), 1);
      drawClosedLoop(p, halb(p, petalPoints(262, 350, p.step * 0.172, 14, 0.6)), 1);
      drawClosedLoop(p, petalPoints(346, 406, p.step * 0.153, 12, 0.6), 1);
      drawPerlen(p, 112, p.step * 0.248, 2, 7, 1.1, p.step / 2);
      drawPerlen(p, 212, p.step * 0.229, 2, 7, 1.1);
      drawPerlen(p, 308, p.step * 0.21, 2, 7, 1.1, p.step / 2);
      drawClosedLoop(p, petalPoints(14, 44, p.step * 0.88, 10, 0.8), 1.5);
    }
  },
  {
    id: 'kordelstern', world: 'geo', axes: 8,
    name: 'Kordelstern', note: 'Achtstrahliger Stern aus drei Dreiecken, Zopf um den Kern',
    mythos: 'Ein Stern ist keine Form, sondern eine Behauptung: dass es eine Mitte gibt und dass alles von dort kommt. Wer lange genug ausmalt, glaubt es irgendwann.',
    build: function (p) {
      drawRing(p, 250, 1.3);
      drawRing(p, 120, 1.3);
      drawClosedLoop(p, trianglePoints(122, 408, p.step * 0.484), 2);
      drawClosedLoop(p, konturPoints(trianglePoints(122, 408, p.step * 0.484), 0.68), 1.2);
      drawClosedLoop(p, halb(p, trianglePoints(122, 406, p.step * 0.484)), 1.4);
      drawClosedLoop(p, halb(p, konturPoints(trianglePoints(122, 406, p.step * 0.484), 0.58)), 1);
      drawWavyRing(p, 84, 24, 16, 1.5);
      drawWavyRing(p, 84, -24, 16, 1.5);
      drawRing(p, 344, 1.1);
      drawClosedLoop(p, halb(p, diamondPoints(254, 342, p.step * 0.30)), 1.2);
      drawClosedLoop(p, halb(p, konturPoints(diamondPoints(254, 342, p.step * 0.30), 0.5)), 0.9);
      drawClosedLoop(p, halb(p, petalPoints(346, 406, p.step * 0.24, 12, 0.8)), 1.2);
      drawSprossen(p, 120, 250, p.step * 0.34, 3, 0.9, p.step / 2);
      drawPerlen(p, 186, p.step * 0.306, 3, 9, 1.2);
      drawPerlen(p, 300, p.step * 0.22, 2, 8, 1.2);
      drawPerlen(p, 376, p.step * 0.19, 2, 7, 1.2);
      drawClosedLoop(p, petalPoints(14, 44, p.step * 0.64, 10, 0.8), 1.5);
    }
  },
  {
    id: 'sprossenband', world: 'feinwerk', axes: 14,
    name: 'Sprossenband', note: 'Fünf Bänder mit Sprossen, jedes mit eigenem Innenmotiv',
    mythos: 'Eine Sprosse ist der kürzeste Weg zwischen zwei Ringen und die einzige Stelle, an der man von einem ins andere kommt. Leitern sind älter als Türen.',
    build: function (p) {
      drawRing(p, 348, 1.3);
      drawRing(p, 276, 1.3);
      drawRing(p, 200, 1.3);
      drawRing(p, 126, 1.3);
      drawSprossen(p, 46, 126, p.step * 0.25, 3, 1.1);
      drawSprossen(p, 126, 200, p.step * 0.25, 3, 1.1, p.step / 2);
      drawSprossen(p, 200, 276, p.step * 0.25, 3, 1.1);
      drawSprossen(p, 276, 348, p.step * 0.25, 3, 1.1, p.step / 2);
      drawSprossen(p, 348, 408, p.step * 0.25, 3, 1.1);
      drawClosedLoop(p, halb(p, petalPoints(128, 198, p.step * 0.20, 14, 0.8)), 1.3);
      drawClosedLoop(p, petalPoints(202, 274, p.step * 0.20, 14, 0.8), 1.3);
      drawClosedLoop(p, halb(p, petalPoints(278, 346, p.step * 0.20, 14, 0.8)), 1.3);
      drawClosedLoop(p, diamondPoints(350, 406, p.step * 0.22), 1.3);
      drawClosedLoop(p, halb(p, diamondPoints(48, 124, p.step * 0.245)), 1.3);
      drawPerlen(p, 163, 0, 1, 8, 1.1);
      drawPerlen(p, 312, 0, 1, 8, 1.1);
      drawClosedLoop(p, petalPoints(14, 42, p.step * 0.9, 10, 0.8), 1.4);
    }
  },
  {
    id: 'tauwerk', world: 'feinwerk', axes: 8,
    name: 'Tauwerk', note: 'Drei Zöpfe auf drei Radien, dazwischen Rauten und Blätter',
    mythos: 'Ein Tau ist stärker als der Draht, aus dem es besteht — nicht, weil mehr Draht darin ist, sondern weil keiner allein zieht.',
    build: function (p) {
      drawWavyRing(p, 140, 46, 8, 1.8);
      drawWavyRing(p, 140, -46, 8, 1.8);
      drawWavyRing(p, 250, 52, 16, 1.8);
      drawWavyRing(p, 250, -52, 16, 1.8);
      drawWavyRing(p, 360, 44, 24, 1.6);
      drawWavyRing(p, 360, -44, 24, 1.6);
      drawRing(p, 306, 1);
      drawRing(p, 196, 1);
      drawClosedLoop(p, diamondPoints(198, 304, p.step * 0.255), 1.3);
      drawClosedLoop(p, konturPoints(diamondPoints(198, 304, p.step * 0.255), 0.5), 0.9);
      drawClosedLoop(p, halb(p, diamondPoints(50, 138, p.step * 0.33)), 1.3);
      drawClosedLoop(p, petalPoints(308, 408, p.step * 0.229, 14, 0.85), 1.3);
      drawClosedLoop(p, konturPoints(petalPoints(308, 408, p.step * 0.229, 14, 0.85), 0.5), 0.9);
      drawPerlen(p, 170, p.step * 0.255, 3, 8, 1.2, p.step / 2);
      drawClosedLoop(p, petalPoints(14, 44, p.step * 0.62, 10, 0.8), 1.5);
    }
  },
  {
    id: 'zellenwerk', world: 'feinwerk', axes: 12,
    name: 'Zellenwerk', note: 'Zellenschmelz: vier Bänder gelöteter Zellen mit eigenem Grund',
    mythos: 'Der Goldschmied lötet erst die Wände und füllt dann die Farbe hinein. Er entscheidet die Ordnung, bevor er weiß, welche Farbe er hat — und das ist keine Not, sondern die Kunst.',
    build: function (p) {
      /* Keine eigenen Ringe an den Bandgrenzen: Die Bänder ziehen ihre
         Kanten selbst. Ein Ring zwei Punkte daneben ergibt einen Spalt
         unter Pixelbreite, und der zerfällt in hunderte Splitter statt in
         einen Ring - nachgemessen bei r≈123, 205 und 308. */
      drawZellenband(p, 46, 128, p.step * 0.5, 2, 1.3);
      drawZellenband(p, 128, 212, p.step * 0.5, 3, 1.3, p.step / 2);
      drawZellenband(p, 212, 300, p.step * 0.5, 3, 1.3);
      drawZellenband(p, 300, 410, p.step * 0.5, 4, 1.3, p.step / 2);
      drawPerlen(p, 168, p.step * 0.2, 2, 7, 1.1, p.step / 2);
      drawPerlen(p, 352, p.step * 0.18, 2, 6, 1.1);
      drawClosedLoop(p, petalPoints(14, 44, p.step * 0.86, 10, 0.8), 1.5);
    }
  },
  {
    id: 'flechtband', world: 'feinwerk', axes: 10, frame: false,
    name: 'Flechtband', note: 'Zwei Wellen, die einander abwechselnd über- und unterqueren',
    mythos: 'An jeder Kreuzung entscheidet sich, wer oben liegt — und weil es sich zwanzigmal entscheidet, liegt am Ende keiner oben. Das ist die ganze Kunst des Flechtens und vermutlich auch die des Zusammenlebens.',
    build: function (p) {
      /* Eigener Rahmen, und beide Zahlen sind gemessen. Die Flechtzone
         braucht eine radiale Teilung: Die Stränge berühren die Ringe bei
         168 und 332 nur tangential, dichten also nicht ab - ohne Speiche
         quer hindurch ist die ganze Zone EIN Feld, und das größte lag bei
         45 Prozent der Scheibe.

         Die Speiche darf hier auch durchlaufen. Bei zehn Achsen und zehn
         Wellen liegt sie bei 0 (mod 0,157·2), die Kreuzungen liegen bei
         0,157 - die Speiche trifft also die Scheitel, nie eine Kreuzung,
         und das Radieren reicht nur ±0,094 um jede Kreuzung. Sie wird
         nicht durchtrennt. Am Scheitel liest sie sich als Bund, der die
         Flechte zusammenhält. */
      drawRing(p, R_OUT, 2.6);
      drawRing(p, 332, 1.6);
      drawRing(p, 168, 1.6);
      drawRing(p, R_IN, 2);
      drawFlechtband(p, 250, 60, 10, 22, 1.8);
      /* Erst jetzt: Was vor dem Radieren gezeichnet wird, radiert das
         Radieren mit weg. Die Speiche muss danach kommen. */
      /* Zwei Bünde je Segment: Bei zehn Achsen und zehn Wellen liegt alle
         0,314 rad ein Scheitel, das Segment ist 0,628 breit. Mit nur einem
         blieb ein Feld von 4,4 Prozent stehen - die Grenze des Prüflaufs
         liegt bei 4. */
      p.ctx.lineWidth = 1.6;
      p.repeat(function () {
        tracePath(p.ctx, [pol(R_IN, UP + p.step / 2), pol(R_OUT, UP + p.step / 2)], false);
        tracePath(p.ctx, [pol(168, UP), pol(332, UP)], false);
      });
      drawClosedLoop(p, petalPoints(48, 166, p.step * 0.36, 16, 0.85), 1.5);
      drawClosedLoop(p, konturPoints(petalPoints(48, 166, p.step * 0.36, 16, 0.85), 0.55), 1);
      drawClosedLoop(p, halb(p, petalPoints(334, 408, p.step * 0.30, 12, 0.8)), 1.4);
      drawClosedLoop(p, halb(p, konturPoints(petalPoints(334, 408, p.step * 0.30, 12, 0.8), 0.52)), 0.95);
      drawSprossen(p, 334, 408, p.step * 0.34, 3, 0.9);
      drawSprossen(p, 46, 166, p.step * 0.34, 3, 0.9, p.step / 2);
      drawPerlen(p, 372, p.step * 0.16, 2, 8, 1.2);
      drawPerlen(p, 110, p.step * 0.20, 2, 7, 1.1);
      drawPerlen(p, 250, 0, 1, 9, 1.2);
      drawClosedLoop(p, petalPoints(14, 44, p.step * 0.72, 10, 0.8), 1.5);
    }
  },
  {
    id: 'kettenglied', world: 'feinwerk', axes: 10,
    name: 'Kettenglied', note: 'Zehn Glieder, jedes zweite über seinen Nachbarn gelegt',
    mythos: 'Ein Glied für sich ist ein Loch mit einem Rand darum. Erst das nächste macht daraus eine Kette — und keines von beiden weiß, welches das erste war.',
    build: function (p) {
      const aussen = ovalPoints(250, 100, p.step * 0.56, 44);
      const innen  = ovalPoints(250, 72,  p.step * 0.38, 44);
      /* Erst alle Glieder, dann die geraden noch einmal: Vor dem Nachziehen
         wird ihr Ringkörper ausradiert, und was darunter lag, ist damit
         unterbrochen. So geht jedes zweite Glied sichtbar über den Nachbarn. */
      p.repeat(function () {
        p.ctx.lineWidth = 1.8;
        tracePath(p.ctx, aussen, true);
        tracePath(p.ctx, innen, true);
      });
      for (let i = 0; i < p.axes; i += 2) {
        const dreh = i * p.step;
        radieren(p, 3.6, function () {
          tracePath(p.ctx, rotatePoints(aussen, dreh), true);
          tracePath(p.ctx, rotatePoints(innen, dreh), true);
        });
        p.ctx.lineWidth = 0.1;
        p.ctx.save();
        p.ctx.globalCompositeOperation = 'destination-out';
        p.ctx.beginPath();
        const A = rotatePoints(aussen, dreh), I = rotatePoints(innen, dreh);
        p.ctx.moveTo(A[0][0], A[0][1]);
        for (let k = 1; k < A.length; k++) p.ctx.lineTo(A[k][0], A[k][1]);
        p.ctx.closePath();
        p.ctx.moveTo(I[0][0], I[0][1]);
        for (let k = I.length - 1; k >= 0; k--) p.ctx.lineTo(I[k][0], I[k][1]);
        p.ctx.closePath();
        p.ctx.fill('evenodd');
        p.ctx.restore();
        p.ctx.lineWidth = 1.8;
        tracePath(p.ctx, A, true);
        tracePath(p.ctx, I, true);
      }
      /* In das Loch jedes Gliedes kommt Inhalt - sonst ist ein Glied ein
         einziges großes Feld, und die Kette hat zehn davon. */
      drawClosedLoop(p, petalPoints(196, 302, p.step * 0.26, 14, 0.85), 1.2);
      drawPerlen(p, 250, p.step * 0.16, 2, 8, 1.2);
      drawRing(p, 140, 1.3);
      drawRing(p, 362, 1.3);
      drawClosedLoop(p, petalPoints(46, 138, p.step * 0.42, 14, 0.85), 1.4);
      drawClosedLoop(p, konturPoints(petalPoints(46, 138, p.step * 0.42, 14, 0.85), 0.52), 0.95);
      drawClosedLoop(p, halb(p, petalPoints(364, 408, p.step * 0.28, 12, 0.8)), 1.3);
      drawSprossen(p, 362, 408, p.step * 0.32, 3, 0.9);
      drawPerlen(p, 160, p.step * 0.2, 2, 7, 1.1, p.step / 2);
      drawClosedLoop(p, petalPoints(14, 44, p.step * 0.72, 10, 0.8), 1.5);
    }
  },
  {
    id: 'schneckenwerk', world: 'feinwerk', axes: 8,
    name: 'Schneckenwerk', note: 'Paarweise gegenläufige Schnecken, die Signatur der Filigranarbeit',
    mythos: 'Der Draht wird nicht gebogen, bis er eine Form hat — er wird gewickelt, bis er nicht mehr weiterkann. Was dabei entsteht, hat niemand entworfen, und trotzdem sieht es aus wie eine Absicht.',
    build: function (p) {
      drawRing(p, 232, 1.2);
      drawRing(p, 120, 1.2);
      drawRing(p, 340, 1.2);
      [1, -1].forEach(function (vz) {
        drawPolyline(p, spiralPoints(122, 230, vz * 1.55, 60), 1.7);
        drawPolyline(p, halb(p, spiralPoints(234, 338, vz * 1.10, 50)), 1.5);
        drawPolyline(p, spiralPoints(46, 118, vz * 1.30, 40), 1.3);
      });
      drawPerlen(p, 176, p.step * 0.10, 1, 13, 1.4);
      drawPerlen(p, 176, p.step * 0.10, 1, 6, 1.2);
      drawPerlen(p, 286, p.step * 0.08, 1, 11, 1.3, p.step / 2);
      drawClosedLoop(p, halb(p, petalPoints(342, 408, p.step * 0.30, 14, 0.8)), 1.4);
      drawClosedLoop(p, halb(p, konturPoints(petalPoints(342, 408, p.step * 0.30, 14, 0.8), 0.52)), 0.95);
      drawSprossen(p, 340, 408, p.step * 0.40, 3, 0.9);
      drawClosedLoop(p, petalPoints(14, 44, p.step * 0.62, 10, 0.8), 1.5);
    }
  },
  {
    id: 'nadelkrone', world: 'feinwerk', axes: 16,
    name: 'Nadelkrone', note: 'Sechzehn lange Nadeln mit Innenkontur, im Außenband Blätter mit Kern',
    mythos: 'Eine Krone aus Nadeln setzt man niemandem auf. Man stellt sie hin und geht darum herum, und das genügt.',
    build: function (p) {
      drawRing(p, 344, 1.2);
      drawRing(p, 250, 1.2);
      drawRing(p, 120, 1.2);
      drawClosedLoop(p, trianglePoints(122, 342, p.step * 0.28), 1.6);
      drawClosedLoop(p, konturPoints(trianglePoints(122, 342, p.step * 0.28), 0.52), 0.95);
      drawClosedLoop(p, halb(p, trianglePoints(122, 248, p.step * 0.28)), 1.3);
      drawClosedLoop(p, halb(p, diamondPoints(252, 342, p.step * 0.23)), 1.1);
      drawSprossen(p, 250, 344, p.step * 0.23, 3, 0.85, p.step / 2);
      drawClosedLoop(p, petalPoints(346, 408, p.step * 0.18, 12, 0.8), 1.3);
      drawClosedLoop(p, konturPoints(petalPoints(346, 408, p.step * 0.18, 12, 0.8), 0.5), 0.9);
      drawClosedLoop(p, halb(p, petalPoints(48, 118, p.step * 0.26, 12, 0.8)), 1.2);
      drawPerlen(p, 378, p.step * 0.12, 1, 9, 1.2, p.step / 2);
      drawPerlen(p, 186, p.step * 0.12, 1, 8, 1.2, p.step / 2);
      drawClosedLoop(p, petalPoints(14, 42, p.step * 1.02, 10, 0.8), 1.4);
    }
  }
);
...
MOTIFS.forEach(function (motiv) {
  ...
  if (motiv.mythos) motiv.mythos = T(motiv.mythos);
...
const Gallery = {
  ...
  open: function () {
    ...
    return new Promise(function (resolve) {
      ...
      let request;
      try {
        request = indexedDB.open('mandala-atelier-regal2-36', 1);
      } catch (err) {
        return resolve();
      }
...
function loadBackup(file) {
  ...
  reader.onload = function () {
    ...
    /* Aus einer älteren Sicherung kann eine längere eigene Farbwelt kommen –
       übernommen wird, was passt (siehe loadOwnPalette). */
    if (Array.isArray(backup.ownPalette) && backup.ownPalette.length) {
      ownPalette().colors = OWN_DEFAULT.map(function (vorgabe, i) {
        const c = backup.ownPalette[i];
        return c && c.hex ? { name: c.name || vorgabe.name, hex: c.hex } : vorgabe;
      });
      Store.set('ownPalette', ownPalette().colors);
      buildPalette();
    }
...
function cacheUi() {
  ...
  /* Beide Stellen: die Kopfzeile und die schwebende Leiste im Vollbild.
     Ein Homescreen-Start läuft im Vollbild – die Nummer stünde sonst genau
     dort nicht, wo am ehesten jemand nachsieht. */
  ['fassung', 'fassung-voll'].forEach(function (id) {
    const el = document.getElementById(id);
    if (el) el.textContent = FASSUNG;
  });
...
function loadOwnPalette() {
  ...
  /* Früher musste die Länge genau stimmen. Als die Welten von vierzehn auf
     zehn Töne kamen, hätte das jede selbst gemischte Farbwelt still verworfen.
     Jetzt wird übernommen, was passt, und der Rest kommt aus der Vorgabe. */
  ownPalette().colors = OWN_DEFAULT.map(function (vorgabe, i) {
    const c = stored[i];
    return c && c.hex
      ? { name: c.name || vorgabe.name, hex: c.hex }
      : { name: vorgabe.name, hex: vorgabe.hex };
  });
}
...
/* ---------------------------------------------------------------------------
   ...
   Der alte Weg war ein `<a download>` samt link.click(). Auf einem Rechner
   tut das, was es soll. Auf dem iPad tut es **nichts** — und zwar still: kein
   Fehler, keine Meldung, kein Bild. Safari kennt das download-Attribut für
   fremde Ursprünge nicht, und eine vom Homebildschirm gestartete App hat
   nicht einmal eine Adresszeile, in der ein Blob-Ziel landen könnte. Genau da
   wird diese App benutzt, also war „Als Bild speichern“ dort nie mehr als
   eine Beschriftung.

   Der Weg, den es auf dem iPad wirklich gibt, ist das **Teilen-Blatt**:
   navigator.share() mit einer Datei öffnet die Auswahl, in der „Sichern in
   Fotos“ und „In Dateien sichern“ stehen. Damit ist auch gleich der Weg auf
   ein zweites Gerät offen, ohne dass wir irgendetwas verschicken — geteilt
   wird vom Betriebssystem, nicht von uns.

   Zwei Dinge sind daran heikel:

   1. Teilen verlangt eine **frische Berührung**. Wer erst eine Datenbank
      liest und danach teilt, kann die Erlaubnis verloren haben; Safari wirft
      dann NotAllowedError. Deshalb wird der Fehler abgefangen und der Weg
      über den Download versucht, statt stumm zu scheitern.
   2. Ein Abbruch durch die Person ist **kein Fehler**. AbortError bedeutet:
      Teilen-Blatt gesehen, „Abbrechen“ getippt. Dann ist nichts zu melden. */
function dataUrlZuBlob(dataUrl) {
  const komma = dataUrl.indexOf(',');
  const kopf = dataUrl.slice(0, komma);
  const typ = (kopf.match(/:(.*?);/) || [null, 'application/octet-stream'])[1];
...
function gibDateiAus(blob, name, wort) {
  const datei = new File([blob], name, { type: blob.type });

  const herunterladen = function () {
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.download = name;
    link.href = url;
    /* Am Dokument hängen, sonst zählt der Klick in manchen Browsern nicht. */
  ...
  if (navigator.canShare && navigator.canShare({ files: [datei] })) {
    return navigator.share({ files: [datei], title: name })
      .then(function () { say(wort + ' weitergegeben.'); })
      .catch(function (err) {
...
function start() {
  ...
  buildPalette();
  /* Ein gespeicherter Ton kann seit dem letzten Öffnen aus der Welt geflogen
     sein (Auslese der zu nahen Pigmente). Dann fällt die Wahl zurück auf den
     ersten – sonst zeigt die Leiste keinen Stift als gewählt an, und
     mixPigment() fände seinen Platz in der eigenen Welt nicht wieder. */
  state.color  = Store.get('color', pigments()[0].hex);
  if (!pigments().some(function (p) { return p.hex === state.color; })) {
    state.color = pigments()[0].hex;
  }
