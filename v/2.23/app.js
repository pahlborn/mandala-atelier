...
/* Fünf Farbwelten zu zehn bis vierzehn Pigmenten. Alle gedeckt – keine
   Buntstift-Knallfarben; das trägt die erwachsene Anmutung wesentlich. Die
   Reihenfolge ist nicht beliebig: die ersten zehn Einträge jeder Welt sind
   bewusst gut unterscheidbar, weil die Farblegende der Zählmandalas sie der
   Reihe nach vergibt.

   Warum die Welten verschieden groß sind: Die Zahl ist gemessen, nicht
   gesetzt. Sieben Töne sind ausgelesen worden, weil sie einem anderen
   derselben Welt zu nahe lagen — Raureif, Reseda, Perle, Rauch, Asche,
   Altrosa und der Kalk des Goldgrunds. Gemessen wurde als CIEDE2000 **bei
   70 % Deckung auf dem Papierton**, nicht am reinen Pigment: Blatt reibt die
   Farbe durch, und dabei schrumpft jeder Abstand auf etwa sieben Zehntel.
   Perle und Kalk standen so bei ΔE 2.9 – auf dem Blatt dieselbe Fläche.
   Jetzt liegt in keiner Welt ein Paar unter ΔE 7.

   Sechs der sieben waren Nachzügler: Blatt bekam in 3.8 die Welten des
   Ateliers und wurde dafür von neun auf vierzehn Töne je Welt aufgefüllt.
   Aufgefüllt wurde nach Stimmung, nicht nach Abstand — das ist der Fehler,
...
const PALETTES = [
  ...
  /* Goldgrund – als einzige Welt keine Sammlung von Tönen, sondern **Leitern**.
     ...
     Hier stehen drei: Gold in fünf Stufen, Indigo in vier, Purpur in drei,
     dazu Grund als dunkles Ende. Die Stufen liegen 14 bis 22 L* auseinander –
     gleichmäßig genug, dass eine Folge als Verlauf liest. Das helle Ende ist
     Lichtgold; ein zusätzlicher Kalk stand ihm mit ΔE 6.4 so nahe, dass beide
     dieselbe Fläche zu färben schienen (siehe Abschnitt zur Auslese).
...
const Store = {
  prefix: 'mandala-atelier-regal2-23.',
  get(key, fallback) {
    try {
...
const Gallery = {
  ...
  open: function () {
    ...
    return new Promise(function (resolve) {
      ...
      let request;
      try {
        request = indexedDB.open('mandala-atelier-regal2-23', 1);
      } catch (err) {
        return resolve();
      }
